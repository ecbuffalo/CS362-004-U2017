...
int playVillage(struct gameState *state, int handPos, int currentPlayer) {
    drawCard(currentPlayer, state);
    state->numActions = state->numActions + 1;//bug 1
    discardCard(handPos, currentPlayer, state, 0);
    return 0;
}
...
int playAdventurer(struct gameState *state, int currentPlayer, int *temphand, int drawntreasure, int cardDrawn, int z) {
    while (drawntreasure < 2) {
        ...
        drawCard(currentPlayer, state);
        cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer] - 0];//bug 2 //top card of hand is most recently drawn card.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
    ...
    switch (card) {
        case adventurer:
            return playAdventurer(state, currentPlayer, temphand, drawntreasure, cardDrawn, z);
        ...
        case steward:
            return playSteward(choice1, choice2, choice3, state, currentPlayer, handPos);
